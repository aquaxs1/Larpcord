...
console.log("Larpcord v" + app.getVersion());
...
function init() {
    ...
    app.whenReady().then(async () => {
        if (process.platform === "win32") app.setAppUserModelId("dev.larpcord.app");
...
if (!app.requestSingleInstanceLock({ IS_DEV })) {
    if (IS_DEV) {
        console.log("Larpcord is already running. Quitting previous instance...");
        init();
    } else {
        console.log("Larpcord is already running. Quitting...");
        app.quit();
    }
} else {
    init();
